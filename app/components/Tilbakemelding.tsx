...
export default function Tilbakemelding() {
  ...
  return <>
    ...
    {apen && <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 p-4" role="dialog" aria-modal="true" aria-label="Send tilbakemelding">
      <form onSubmit={send} className="mx-auto mt-12 max-w-lg rounded-3xl bg-white p-6 text-slate-900 shadow-2xl sm:mt-24 sm:p-8">
        ...
        <label className="mt-6 block text-sm font-semibold">Hva gjelder det?<select value={type} onChange={(e) => setType(e.target.value)} className="mt-2 w-full rounded-xl border bg-white px-4 py-3 font-normal text-slate-900"><option value="feil">En feil</option><option value="forslag">Et forslag</option><option value="annet">Annet</option></select></label>
        <label className="mt-4 block text-sm font-semibold">Melding<textarea required minLength={5} rows={5} value={melding} onChange={(e) => setMelding(e.target.value)} className="mt-2 w-full rounded-xl border bg-white px-4 py-3 font-normal text-slate-900 placeholder:text-slate-400" placeholder="Beskriv feilen eller ønsket ditt…" /></label>
        <label className="mt-4 block text-sm font-semibold">E-post (valgfritt)<input type="email" value={epost} onChange={(e) => setEpost(e.target.value)} className="mt-2 w-full rounded-xl border bg-white px-4 py-3 font-normal text-slate-900 placeholder:text-slate-400" placeholder="Hvis du ønsker svar" /></label>
