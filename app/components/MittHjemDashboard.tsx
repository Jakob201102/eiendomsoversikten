...
import { lesAltOmBoligen, type Historikkinfo, type Kontaktinfo, type Rominfo } from "../lib/alt-om-boligen";
...
import { dokumentasjonstyper, kravForArbeidstype, navnPaArbeidstype, type Dokumentasjonstype } from "../lib/dokumentasjonskrav";
import { fullforVedlikeholdsoppgave, oppdaterVedlikeholdsoppgave, opprettVedlikeholdsoppgave, slettVedlikeholdsoppgave, type Vedlikeholdsdata } from "../lib/vedlikehold";

type Visning = "valg" | "historikk" | "vedlikehold" | "dokument" | "bilder" | "fullfor" | "boliginfo" | "dokumentasjon" | null;
type HistorikkSkjema = { tittel: string; dato: string; omrade: string; romId: string; kostnad: string; utfortAv: "selv" | "firma"; firma: string; handverkerId: string; nyHandverkerNavn: string; beskrivelse: string; arbeidstype: Dokumentasjonstype; dokumentasjonBekreftet: string[] };
...
const tomHistorikk: HistorikkSkjema = { tittel: "", dato: new Date().toISOString().slice(0, 10), omrade: "", romId: "", kostnad: "", utfortAv: "selv", firma: "", handverkerId: "", nyHandverkerNavn: "", beskrivelse: "", arbeidstype: "", dokumentasjonBekreftet: [] };
const tomVedlikehold: VedlikeholdSkjema = { tittel: "", frist: "", omrade: "", kostnad: "", notat: "" };
const vedlikeholdsomrader = ["Kjøkken", "Bad", "Stue", "Soverom", "Kjeller", "Loft", "Tak", "Fasade", "Hage", "Garasje", "Annet"];
...
export default function MittHjemDashboard({
  ...
  const [dokumentasjonHendelse, setDokumentasjonHendelse] = useState<Historikkinfo | null>(null);
  ...
  async function lagreHistorikk() {
    ...
    try {
      ...
      if (handverkerId === "ny" && historikkSkjema.nyHandverkerNavn.trim()) { handverkerId = crypto.randomUUID(); kontakter = [...data.kontakter, { id: handverkerId, navn: historikkSkjema.nyHandverkerNavn.trim(), kategori: "Annet", telefon: "", epost: "", nettside: "", notat: "Opprettet fra bolighistorikken." }]; }
      const valgtHandverker = kontakter.find((kontakt) => kontakt.id === handverkerId);
      const hendelse: Historikkinfo = { id, dato, tittel: historikkSkjema.tittel.trim(), omrade: historikkSkjema.omrade, romId: historikkSkjema.romId, kostnad: Number(historikkSkjema.kostnad) || 0, utfortAv: historikkSkjema.utfortAv, firma: historikkSkjema.utfortAv === "firma" ? (valgtHandverker?.navn || historikkSkjema.firma.trim()) : "", handverkerId: handverkerId === "ny" ? "" : handverkerId, beskrivelse: historikkSkjema.beskrivelse.trim(), dokumentIder: vedlegg.dokumentIder, bildeIder: vedlegg.bildeIder, kildeVedlikeholdId: fullforer ? String(fullforer.id) : "", arbeidstype: historikkSkjema.arbeidstype, dokumentasjonBekreftet: historikkSkjema.dokumentasjonBekreftet };
      ...
      await oppdaterBolig(String(bolig.id), { ...bolig, altOmBoligen: { ...data, kontakter, historikk: nyHistorikk, oppdatert: new Date().toISOString() } });
      if (fullforer) await fullforVedlikeholdsoppgave(fullforer, { ferdigdato: dato, faktiskKostnad: Number(historikkSkjema.kostnad) || 0 });
  ...
  async function lagreDokumentasjon() {
    ...
    setJobber(true); setFeil("");
    try {
      const vedlegg = await lastOppVedlegg(dokumentasjonHendelse.id, dokumentasjonHendelse.dato || new Date().toISOString().slice(0, 10));
      const oppdatertHendelse: Historikkinfo = {
        ...dokumentasjonHendelse,
        dokumentIder: [...dokumentasjonHendelse.dokumentIder, ...vedlegg.dokumentIder],
      ...
      await oppdaterBolig(String(bolig.id), {
        ...
        altOmBoligen: {
          ...data,
          historikk: data.historikk.map((hendelse) => hendelse.id === oppdatertHendelse.id ? oppdatertHendelse : hendelse),
          oppdatert: new Date().toISOString(),
        },
      });
      setVisning(null); setDokumentasjonHendelse(null); setBilder([]); setFiler([]); await onOppdatert();
    } catch { setFeil("Kunne ikke oppdatere dokumentasjonen."); } finally { setJobber(false); }
  }
  ...
  return <>
    ...
    <div className="mt-5 grid gap-5 lg:grid-cols-2">
      <Dashboardkort ikon="🔨" tittel="Boligens historikk" lenke={`/bolighistorikk?bolig=${bolig.id}`} lenketekst="Åpne historikkarkivet">
        {historikk.length ? <div className="divide-y divide-stone-100">{historikk.slice(0, MAKS_OVERSIKT).map((hendelse) => <Historikkrad key={hendelse.id} hendelse={hendelse} dokumenter={boligDokumenter} onAapne={aapneDokument} onDokumentasjon={() => startDokumentasjon(hendelse)} kanOppdatere={kanRedigere && data.historikk.some((lagret) => lagret.id === hendelse.id)} />)}</div> : <Tom tekst="Ingen historikk ennå. Registrer det første arbeidet som er utført." knapp="+ Registrer utført arbeid" onClick={() => aapne("historikk")} visKnapp={kanRedigere} />}
      </Dashboardkort>
      <Dashboardkort ikon="📅" tittel="Kommende vedlikehold" lenke="/vedlikehold" lenketekst="Se hele planen">
        {kommende.length ? <div className="space-y-3">{kommende.slice(0, MAKS_OVERSIKT).map((oppgave) => <article key={oppgave.id} className="rounded-xl border border-stone-200 p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-bold">{String(oppgave.tittel || oppgave.navn || "Vedlikehold")}</h3><p className="mt-1 text-sm text-slate-500">{oppgave.frist ? formatDato(String(oppgave.frist)) : "Ingen dato"}{oppgave.omrade ? ` · ${String(oppgave.omrade)}` : ""}</p></div></div>{kanRedigere && <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-sm font-semibold"><button type="button" onClick={() => startFullforing(oppgave)} className="text-emerald-700">Marker som utført</button><button type="button" onClick={() => redigerOppgave(oppgave)} className="text-slate-600">Rediger</button><button type="button" onClick={() => slettOppgave(oppgave)} className="text-red-600">Slett</button></div>}</article>)}</div> : <Tom tekst="Du har ingen kommende oppgaver." knapp="+ Legg til vedlikehold" onClick={() => aapne("vedlikehold")} visKnapp={kanRedigere} />}
      </Dashboardkort>
      <Dashboardkort ikon="📁" tittel="Dokumenter" lenke="/dokumentarkiv" lenketekst="Åpne dokumentarkivet"><Filrader filer={dokumenterIAarkiv.slice(0, MAKS_OVERSIKT)} onAapne={aapneDokument} tomtekst="Ingen dokumenter er lagret." />{kanRedigere && <button type="button" onClick={() => aapne("dokument")} className="mt-4 text-sm font-bold text-emerald-700">+ Legg til dokument</button>}</Dashboardkort>
      <Dashboardkort ikon="📸" tittel="Bilder" lenke={`/alt-om-boligen?bolig=${bolig.id}`} lenketekst="Se boligdetaljer"><Filrader filer={bilderIAarkiv.slice(0, MAKS_OVERSIKT)} onAapne={aapneDokument} tomtekst="Ingen bilder er lagret." />{kanRedigere && <button type="button" onClick={() => aapne("bilder")} className="mt-4 text-sm font-bold text-emerald-700">+ Legg til bilder</button>}</Dashboardkort>
    </div>

    {visning && <Modal tittel={modaltittel(visning, Boolean(fullforer), Boolean(redigererOppgaveId))} onLukk={() => { setVisning(null); setFullforer(null); setDokumentasjonHendelse(null); setRedigererOppgaveId(""); setFeil(""); }}>
      {feil && <p className="mb-4 rounded-xl bg-red-50 p-3 text-sm text-red-700">{feil}</p>}
      {visning === "valg" && <div className="grid gap-3 sm:grid-cols-2"><Valg ikon="🔨" tittel="Utført arbeid" tekst="Oppussing, bytte eller reparasjon utført av deg eller en håndverker" onClick={() => setVisning("historikk")} /><Valg ikon="📅" tittel="Fremtidig vedlikehold" tekst="Noe som skal gjøres senere" onClick={() => setVisning("vedlikehold")} /><Valg ikon="📄" tittel="Dokument" tekst="Faktura, kvittering eller rapport" onClick={() => setVisning("dokument")} /><Valg ikon="📸" tittel="Bilder" tekst="Ta eller last opp bilder" onClick={() => setVisning("bilder")} /></div>}
      {(visning === "historikk" || visning === "fullfor") && <><Historikkskjema verdi={historikkSkjema} onEndre={setHistorikkSkjema} rom={data.rom} kontakter={data.kontakter} bilder={bilder} filer={filer} setBilder={setBilder} setFiler={setFiler} /><Dokumentasjonsoppsett verdi={historikkSkjema} onEndre={setHistorikkSkjema} /><Lagre onClick={lagreHistorikk} jobber={jobber} tekst={visning === "fullfor" ? "Marker som utført og lagre" : "Lagre i historikken"} /></>}
      ...
      {visning === "dokumentasjon" && dokumentasjonHendelse && <><p className="mb-4 rounded-xl bg-stone-50 p-4 text-sm"><strong>{dokumentasjonHendelse.tittel}</strong><span className="mt-1 block text-slate-500">Dokumentasjonssjekken er frivillig. Velg «Ingen dokumentasjonssjekk» dersom du ikke ønsker varsel.</span></p><Felt label="Dokumentasjonssjekk"><select value={dokumentasjonHendelse.arbeidstype || ""} onChange={(event) => setDokumentasjonHendelse({ ...dokumentasjonHendelse, arbeidstype: event.target.value, dokumentasjonBekreftet: [] })} className="felt">{dokumentasjonstyper.map((valg) => <option key={valg.verdi} value={valg.verdi}>{valg.navn}</option>)}</select></Felt>{dokumentasjonHendelse.arbeidstype && <div className="mt-5"><Dokumentasjonssjekk type={dokumentasjonHendelse.arbeidstype} valgte={dokumentasjonHendelse.dokumentasjonBekreftet || []} onEndre={(valgte) => setDokumentasjonHendelse({ ...dokumentasjonHendelse, dokumentasjonBekreftet: valgte })} /></div>}<div className="mt-5 grid gap-4 sm:grid-cols-2"><Felt label={`Nye bilder${bilder.length ? ` (${bilder.length})` : ""}`}><input type="file" multiple accept="image/*" onChange={(event) => setBilder(Array.from(event.target.files || []))} className="felt text-sm" /></Felt><Felt label={`Nye dokumenter${filer.length ? ` (${filer.length})` : ""}`}><input type="file" multiple accept="image/*,.pdf,.doc,.docx" onChange={(event) => setFiler(Array.from(event.target.files || []))} className="felt text-sm" /></Felt></div><Lagre onClick={lagreDokumentasjon} jobber={jobber} tekst="Lagre" /></>}
    </Modal>}
  </>;
}

function Historikkskjema({ verdi, onEndre, rom, kontakter, bilder, filer, setBilder, setFiler }: { verdi: HistorikkSkjema; onEndre: (verdi: HistorikkSkjema) => void; rom: Rominfo[]; kontakter: Kontaktinfo[]; bilder: File[]; filer: File[]; setBilder: (filer: File[]) => void; setFiler: (filer: File[]) => void }) {
  const valgtOmrade = verdi.romId ? `rom:${verdi.romId}` : verdi.omrade;
  const erTidligereOmrade = Boolean(verdi.omrade && verdi.omrade !== "Annet" && !rom.some((romverdi) => romverdi.navn === verdi.omrade));
  ...
  return <div className="grid gap-4 sm:grid-cols-2"><Felt label="Hva ble gjort? *"><input autoFocus value={verdi.tittel} onChange={(event) => onEndre({ ...verdi, tittel: event.target.value })} placeholder="For eksempel byttet varmtvannsbereder" className="felt" /></Felt><Felt label="Når?"><input type="date" value={verdi.dato} onChange={(event) => onEndre({ ...verdi, dato: event.target.value })} className="felt" /></Felt><Felt label="Hvor på boligen?"><select value={valgtOmrade} onChange={(event) => velgOmrade(event.target.value)} className="felt"><option value="">Velg rom</option>{rom.map((romverdi) => <option key={romverdi.id} value={`rom:${romverdi.id}`}>{romverdi.navn}</option>)}<option value="Annet">Annet</option>{erTidligereOmrade && <option value={verdi.omrade}>{verdi.omrade} (tidligere valg)</option>}</select></Felt><Felt label="Kostnad (valgfritt)"><input inputMode="numeric" value={verdi.kostnad} onChange={(event) => onEndre({ ...verdi, kostnad: event.target.value })} className="felt" /></Felt><Felt label="Utført av"><select value={verdi.utfortAv} onChange={(event) => onEndre({ ...verdi, utfortAv: event.target.value as "selv" | "firma" })} className="felt"><option value="selv">Meg selv</option><option value="firma">Firma/håndverker</option></select></Felt>{verdi.utfortAv === "firma" && <><Felt label="Håndverker"><select value={verdi.handverkerId} onChange={(event) => onEndre({ ...verdi, handverkerId: event.target.value })} className="felt"><option value="">Skriv firmanavn selv</option>{kontakter.map((kontakt) => <option key={kontakt.id} value={kontakt.id}>{kontakt.navn} · {kontakt.kategori}</option>)}<option value="ny">+ Opprett ny håndverker</option></select></Felt>{verdi.handverkerId === "ny" ? <Felt label="Navn på ny håndverker"><input value={verdi.nyHandverkerNavn} onChange={(event) => onEndre({ ...verdi, nyHandverkerNavn: event.target.value })} className="felt" /></Felt> : !verdi.handverkerId && <Felt label="Firma (valgfritt)"><input value={verdi.firma} onChange={(event) => onEndre({ ...verdi, firma: event.target.value })} className="felt" /></Felt>}</>}<label className="text-sm font-semibold sm:col-span-2">Notat<textarea rows={3} value={verdi.beskrivelse} onChange={(event) => onEndre({ ...verdi, beskrivelse: event.target.value })} className="felt mt-2" /></label><Felt label={`Bilder${bilder.length ? ` (${bilder.length})` : ""}`}><input type="file" multiple accept="image/*" capture="environment" onChange={(event) => setBilder(Array.from(event.target.files || []))} className="felt text-sm" /></Felt><Felt label={`Faktura/dokumentasjon${filer.length ? ` (${filer.length})` : ""}`}><input type="file" multiple accept="image/*,.pdf,.doc,.docx" onChange={(event) => setFiler(Array.from(event.target.files || []))} className="felt text-sm" /></Felt></div>;
}

function Dokumentasjonsoppsett({ verdi, onEndre }: { verdi: HistorikkSkjema; onEndre: (verdi: HistorikkSkjema) => void }) {
  return <section className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 sm:p-5"><p className="font-bold">Har du riktig dokumentasjon?</p><p className="mt-1 text-sm text-slate-600">Velg type arbeid for å få en enkel sjekkliste. Dette er valgfritt og kan fylles ut senere.</p><label className="mt-4 block text-sm font-semibold">Type arbeid<select value={verdi.arbeidstype} onChange={(event) => onEndre({ ...verdi, arbeidstype: event.target.value as Dokumentasjonstype, dokumentasjonBekreftet: [] })} className="felt mt-2 bg-white">{dokumentasjonstyper.map((valg) => <option key={valg.verdi} value={valg.verdi}>{valg.navn}</option>)}</select></label>{verdi.arbeidstype && <div className="mt-4"><Dokumentasjonssjekk type={verdi.arbeidstype} valgte={verdi.dokumentasjonBekreftet} onEndre={(valgte) => onEndre({ ...verdi, dokumentasjonBekreftet: valgte })} /></div>}</section>;
}

function Dokumentasjonssjekk({ type, valgte, onEndre }: { type: string | undefined; valgte: string[]; onEndre: (valgte: string[]) => void }) {
  const krav = kravForArbeidstype(type);
  return <div><p className="mb-2 text-xs font-bold uppercase tracking-wide text-amber-800">Anbefalt dokumentasjon</p><div className="space-y-2">{krav.map((punkt) => <label key={punkt.id} className="flex cursor-pointer items-start gap-3 rounded-xl bg-white p-3 text-sm ring-1 ring-amber-100"><input type="checkbox" checked={valgte.includes(punkt.id)} onChange={(event) => onEndre(event.target.checked ? [...valgte, punkt.id] : valgte.filter((id) => id !== punkt.id))} className="mt-0.5 h-5 w-5 rounded border-stone-300 text-emerald-600" /><span>{punkt.navn}</span></label>)}</div><p className="mt-3 text-xs leading-5 text-slate-500">Listen er veiledende. Hva som kreves avhenger av arbeidet og gjeldende regler.</p></div>;
}

function Vedlikeholdsskjema({ verdi, onEndre }: { verdi: VedlikeholdSkjema; onEndre: (verdi: VedlikeholdSkjema) => void }) {
  return <div className="grid gap-4 sm:grid-cols-2"><Felt label="Hva skal gjøres? *"><input autoFocus value={verdi.tittel} onChange={(event) => onEndre({ ...verdi, tittel: event.target.value })} placeholder="For eksempel rense takrenner" className="felt" /></Felt><Felt label="Når?"><input type="date" value={verdi.frist} onChange={(event) => onEndre({ ...verdi, frist: event.target.value })} className="felt" /></Felt><Felt label="Område"><select value={verdi.omrade} onChange={(event) => onEndre({ ...verdi, omrade: event.target.value })} className="felt"><option value="">Velg område</option>{vedlikeholdsomrader.map((omrade) => <option key={omrade}>{omrade}</option>)}</select></Felt><Felt label="Estimert kostnad"><input inputMode="numeric" value={verdi.kostnad} onChange={(event) => onEndre({ ...verdi, kostnad: event.target.value })} className="felt" /></Felt><label className="text-sm font-semibold sm:col-span-2">Notat<textarea rows={3} value={verdi.notat} onChange={(event) => onEndre({ ...verdi, notat: event.target.value })} className="felt mt-2" /></label></div>;
}
...
function Historikkrad({ hendelse, dokumenter, onAapne, onDokumentasjon, kanOppdatere }: { hendelse: Historikkinfo; dokumenter: Dokument[]; onAapne: (dokument: Dokument) => void; onDokumentasjon: () => void; kanOppdatere: boolean }) {
  ...
  return <article className="py-4 first:pt-0 last:pb-0"><p className="text-xs font-bold uppercase tracking-wider text-emerald-700">{maanedAar(hendelse.dato)}</p><h3 className="mt-1 text-lg font-bold">{hendelse.tittel}</h3><p className="mt-1 text-sm text-slate-500">{[hendelse.omrade, hendelse.kostnad ? kroner(hendelse.kostnad) : "", hendelse.firma || (hendelse.utfortAv === "selv" ? "Gjort selv" : "")].filter(Boolean).join(" · ")}</p>{hendelse.beskrivelse && <p className="mt-2 text-sm text-slate-600">{hendelse.beskrivelse}</p>}{krav.length > 0 && <div className={`mt-3 rounded-xl p-3 text-sm ${komplett ? "bg-emerald-50 text-emerald-800" : "bg-amber-50 text-amber-900"}`}><div className="flex flex-wrap items-center justify-between gap-2"><strong>{komplett ? "✓ Dokumentasjonen er komplett" : `Dokumentasjon ${bekreftet.filter((id) => krav.some((punkt) => punkt.id === id)).length} av ${krav.length}`}</strong><span className="text-xs font-semibold">{navnPaArbeidstype(hendelse.arbeidstype)}</span></div>{mangler.length > 0 && <p className="mt-1 text-xs">Mangler: {mangler.map((punkt) => punkt.navn).join(", ")}</p>}{kanOppdatere && <button type="button" onClick={onDokumentasjon} className="mt-2 text-xs font-bold underline">Oppdater dokumentasjon</button>}</div>}{vedlegg.length > 0 && <div className="mt-2 flex flex-wrap gap-3">{vedlegg.map((dokument) => <button key={dokument.id} type="button" onClick={() => onAapne(dokument)} className="text-sm font-semibold text-emerald-700">{dokument.filtype.startsWith("image/") ? "📸" : "📄"} {dokument.navn}</button>)}</div>}{krav.length === 0 && kanOppdatere && <button type="button" onClick={onDokumentasjon} className="mt-3 text-xs font-bold text-amber-700">+ Legg til dokumentasjonssjekk</button>}</article>;
}
...
function Dashboardkort({ ikon, tittel, lenke, lenketekst, children }: { ikon: string; tittel: string; lenke?: string; lenketekst?: string; children: React.ReactNode }) { return <section className="rounded-3xl bg-white p-5 shadow-sm sm:p-6"><div className="mb-5 flex items-center justify-between gap-3"><h2 className="text-xl font-bold"><span className="mr-2">{ikon}</span>{tittel}</h2>{lenke && lenketekst && <Link href={lenke} className="text-xs font-semibold text-slate-500">{lenketekst} →</Link>}</div>{children}</section>; }
function Tom({ tekst, knapp, onClick, visKnapp = true }: { tekst: string; knapp: string; onClick: () => void; visKnapp?: boolean }) { return <div className="rounded-2xl border border-dashed border-stone-300 p-5 text-center"><p className="text-sm text-slate-500">{tekst}</p>{visKnapp && <button type="button" onClick={onClick} className="mt-3 text-sm font-bold text-emerald-700">{knapp}</button>}</div>; }
...
function modaltittel(visning: Visning, fullforer: boolean, redigerer: boolean) { if (visning === "valg") return "Hva vil du legge til?"; if (visning === "historikk" || fullforer) return fullforer ? "Fullfør vedlikehold" : "Registrer utført arbeid"; if (visning === "vedlikehold") return redigerer ? "Rediger vedlikehold" : "Fremtidig vedlikehold"; if (visning === "dokument") return "Legg til dokument"; if (visning === "bilder") return "Legg til bilder"; if (visning === "dokumentasjon") return "Dokumentasjon på arbeidet"; return "Boliginformasjon"; }
