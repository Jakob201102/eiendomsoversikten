...
export default function Navigasjon() {
  ...
  // Supabase-klienten opprettes bare én gang.
  const [supabase] = useState(() => createClient());

  const [menyApen, setMenyApen] = useState(false);
  const [brukerEpost, setBrukerEpost] =
    useState<string | null>(null);
  const [sjekkerBruker, setSjekkerBruker] =
    useState(true);
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex min-h-16 items-center justify-between gap-4">
              ...
              (brukerEpost ? (
                <div className="flex items-center gap-3">
                  <Link
                    ...
                    className={
                      pathname.startsWith("/konto")
                        ? "max-w-36 truncate text-sm font-semibold text-emerald-400"
                        : "max-w-36 truncate text-sm text-slate-400 hover:text-white"
                    }
                  >
                    Min konto
                  </Link>
                  ...
                  <button
                    ...
                    onClick={loggUt}
                    disabled={loggerUt}
                    className="rounded-lg border border-slate-700 px-4 py-2 font-semibold hover:border-red-400 hover:text-red-300 disabled:opacity-50"
                  >
                    {loggerUt
                      ? "Logger ut…"
                      : "Logg ut"}
                ...
                <Link
                  ...
                  className={
                    pathname.startsWith("/logg-inn")
                      ? "rounded-lg bg-emerald-400 px-4 py-2 font-semibold text-slate-950"
                      : "rounded-lg border border-slate-700 px-4 py-2 font-semibold hover:border-emerald-400"
                  }
                >
                  Logg inn
                </Link>
          ...
          <button
            ...
            className="rounded-lg border border-slate-700 px-4 py-2 font-semibold md:hidden"
          >
            {menyApen ? "Lukk" : "Meny"}
          </button>
        </div>
        ...
        {menyApen && (
          <div className="space-y-2 border-t border-slate-800 py-4 md:hidden">
            {lenker.map((lenke) => (
              <Link
                key={lenke.adresse}
                href={lenke.adresse}
              ...
              (brukerEpost ? (
                <div className="space-y-2 border-t border-slate-800 pt-3">
                  ...
                  <Link
                    ...
                    className="block rounded-xl border border-slate-700 px-4 py-3 text-center font-semibold"
                  >
                    Min konto
                  </Link>
