...
type Arkivhendelse = Historikkinfo & {
  arkivtype: Exclude<Arkivfane, "alle">;
...
export default function Bolighistorikk() {
  ...
  const lastInn = useCallback(
    async (beholdId?: string) => {
      ...
      try {
        ...
        const [alle, arkiv] = await Promise.all([
          hentBoliger(),
          hentDokumenter(),
        ]);
        const privateBoliger = alle.filter(
        ...
        setValgtId(
          (gammel) =>
            [onsket, gammel, String(privateBoliger[0]?.id || "")].find((id) =>
              privateBoliger.some((bolig) => String(bolig.id) === id),
            ) || "",
        );
      } catch (error) {
        console.error(error);
        setFeil("Kunne ikke hente bolighistorikken.");
      } finally {
        setLaster(false);
      }
    },
  ...
  const kanRedigere = Boolean(
    bolig && String(bolig.tilgang || "eier") !== "leser",
  );
  const hendelser = useMemo<Arkivhendelse[]>(() => {
    if (!data) return [];
    const resultat: Arkivhendelse[] = data.historikk.map((hendelse) => ({
      ...hendelse,
      arkivtype: hendelse.kildeVedlikeholdId
        ? "vedlikehold"
        : hendelse.kildeSkadeId
          ? "skader"
          : "arbeid",
      kanRedigeres: true,
    }));
  ...
  function startDokumentasjon(hendelse: Arkivhendelse) {
    ...
    setRedigeringsmodus("dokumentasjon");
    setBilder([]);
    setFiler([]);
    setFeil("");
  }
  ...
  async function lagreHendelse() {
    ...
    try {
      const oppdatert = {
        ...redigerer,
        tittel: redigerer.tittel.trim(),
        kostnad: Number(redigerer.kostnad) || 0,
      };
      const finnes = data.historikk.some(
        (hendelse) => hendelse.id === oppdatert.id,
      );
      const historikk = finnes
        ? data.historikk.map((hendelse) =>
            hendelse.id === oppdatert.id ? oppdatert : hendelse,
          )
        : [...data.historikk, oppdatert];
      ...
      setRedigerer(null);
      setRedigeringsmodus(null);
      await lastInn(String(bolig.id));
    } catch {
      setFeil("Kunne ikke lagre endringene.");
    } finally {
      setJobber(false);
    }
  }
  ...
  async function slettHendelse(hendelse: Arkivhendelse) {
    ...
    if (
      !confirm(
        `Slett «${hendelse.tittel}» fra bolighistorikken? Bilder og dokumenter beholdes i dokumentarkivet.`,
      )
    )
      return;
    setJobber(true);
    ...
    try {
      await oppdaterBolig(String(bolig.id), {
        ...
        altOmBoligen: {
          ...data,
          historikk: data.historikk.filter(
            (lagret) => lagret.id !== hendelse.id,
          ),
          oppdatert: new Date().toISOString(),
        },
      });
  ...
  async function lagreDokumentasjon() {
    ...
    try {
      ...
      for (const fil of bilder)
        nyeBilder.push(
          await lastOppDokument(fil, {
            ...
            dokumentdato: dato,
            notat: `Tilhører historikk: ${redigerer.id}`,
          }),
        );
      ...
      const oppdatert = {
        ...redigerer,
        dokumentIder: [...redigerer.dokumentIder, ...nyeDokumenter],
        bildeIder: [...redigerer.bildeIder, ...nyeBilder],
      };
      await oppdaterBolig(String(bolig.id), {
        ...bolig,
        altOmBoligen: {
  ...
  if (laster)
    return (
      <main className="min-h-screen bg-stone-50">
        <Navigasjon />
        <p className="p-12 text-center text-slate-500">Laster historikken…</p>
      </main>
    );
  ...
  return (
    <main className="min-h-screen bg-stone-50 text-slate-900">
      ...
      <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-10">
        <Link
          href={bolig ? `/mitt-hjem?bolig=${bolig.id}` : "/mitt-hjem"}
          className="text-sm font-bold text-emerald-700"
        >
          ← Tilbake til Mitt hjem
        </Link>
        <section className="mt-5 rounded-3xl bg-white p-5 shadow-sm sm:p-7">
          ...
          <div className="mt-1 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h1 className="text-3xl font-bold">Boligens historikk</h1>
              <p className="mt-2 text-slate-500">
                Samlet arkiv over oppussing, arbeid, utført vedlikehold og
                reparerte skader.
              </p>
            </div>
            <div className="flex flex-col gap-3 sm:items-end">
              {boliger.length > 1 && (
                <label className="text-sm font-semibold">
                  ...
                  <select
                    value={valgtId}
                    onChange={(event) => setValgtId(event.target.value)}
                    className="felt mt-2 sm:w-72"
                  >
                    {boliger.map((verdi) => (
                      <option key={verdi.id} value={String(verdi.id)}>
              ...
              {kanRedigere && (
                <button
                  type="button"
                  onClick={startNyHendelse}
                  className="min-h-11 rounded-xl bg-emerald-500 px-5 py-2.5 text-sm font-bold text-white"
                >
                  + Legg til tidligere arbeid
                </button>
              )}
            </div>
          </div>
          {bolig && (
            <p className="mt-4 rounded-xl bg-stone-50 p-3 text-sm font-semibold">
              {String(bolig.adresse || "Privat bolig")} · {hendelser.length}{" "}
              historikkhendelse{hendelser.length === 1 ? "" : "r"}
            </p>
          )}
        </section>
        {feil && (
        ...
        <div className="mt-5 grid grid-cols-2 gap-2 rounded-2xl bg-white p-2 shadow-sm sm:grid-cols-4">
          {(
            [
              ["alle", "Alle"],
              ["arbeid", "Oppussing og arbeid"],
              ["vedlikehold", "Utført vedlikehold"],
              ["skader", "Løste skader"],
            ] as const
          ).map(([verdi, navn]) => (
            <button
              key={verdi}
              type="button"
              onClick={() => setFane(verdi)}
              className={`min-h-11 rounded-xl px-3 py-2 text-sm font-bold ${fane === verdi ? "bg-emerald-500 text-white" : "text-slate-600 hover:bg-stone-50"}`}
            >
              {navn}
            </button>
          ))}
            ...
            <div className="divide-y divide-stone-100">
              {viste.map((hendelse) => (
                <Historikkrad
                  ...
                  onRediger={() => startRedigering(hendelse)}
                  onSlett={() => slettHendelse(hendelse)}
                  kanRedigere={kanRedigere && hendelse.kanRedigeres}
                  jobber={jobber}
                />
              ))}
            </div>
          ) : (
            <p className="rounded-2xl border border-dashed border-stone-300 p-8 text-center text-slate-500">
              Ingen hendelser i denne kategorien ennå.
            </p>
          )}
        </section>
      </div>
      {redigerer && redigeringsmodus === "hendelse" && (
        <Modal
          tittel="Rediger historikkhendelse"
          onLukk={() => {
            setRedigerer(null);
            setRedigeringsmodus(null);
          }}
        >
          ...
          <button
            ...
            disabled={jobber}
            className="mt-6 min-h-12 w-full rounded-xl bg-emerald-500 px-5 py-3 font-bold text-white disabled:opacity-50"
          >
            {jobber ? "Lagrer…" : "Lagre endringer"}
          </button>
        </Modal>
      )}
      {redigerer && redigeringsmodus === "dokumentasjon" && (
        <Modal
          tittel="Oppdater dokumentasjon"
          onLukk={() => {
            setRedigerer(null);
            setRedigeringsmodus(null);
          }}
        >
          <p className="rounded-xl bg-stone-50 p-4">
            <strong>{redigerer.tittel}</strong>
          ...
          {redigerer.arbeidstype && (
            <Sjekkliste
              type={redigerer.arbeidstype}
              valgte={redigerer.dokumentasjonBekreftet || []}
              onEndre={(valgte) =>
                setRedigerer({ ...redigerer, dokumentasjonBekreftet: valgte })
              }
            />
          )}
...
function Historikkrad({
  ...
  const vedlegg = dokumenter.filter((dokument) =>
    [...hendelse.dokumentIder, ...hendelse.bildeIder].includes(dokument.id),
  );
    ...
    <article className="py-5 first:pt-0 last:pb-0">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-xl font-bold">{hendelse.tittel}</h2>
            ...
            {[
              ...
              hendelse.firma ||
                (hendelse.utfortAv === "selv"
                  ? "Gjort selv"
                  : hendelse.utfortAv === "tidligere-eier" ||
                      !hendelse.utfortAv
                    ? "Tidligere eier"
                    : ""),
            ]
          ...
          <div className="flex flex-wrap justify-between gap-2">
            <strong>
              {komplett
                ...
                : `Dokumentasjon ${krav.length - mangler.length} av ${krav.length}`}
            </strong>
      ...
      {kanRedigere && (
        <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-sm font-bold">
          ...
          >
            {krav.length ? "Oppdater dokumentasjon" : "Dokumentasjon"}
          </button>
          <button
            type="button"
            onClick={onSlett}
...
function Hendelsesskjema({
  ...
  rom: Rominfo[];
  onEndre: (hendelse: Historikkinfo) => void;
}) {
  const valgtOmrade = hendelse.romId
    ? `rom:${hendelse.romId}`
    : hendelse.omrade;
  const tidligereOmrade = Boolean(
    hendelse.omrade &&
      hendelse.omrade !== "Annet" &&
      !rom.some((romverdi) => romverdi.navn === hendelse.omrade),
  );
  function velgOmrade(verdi: string) {
    if (verdi.startsWith("rom:")) {
      const romId = verdi.slice(4);
      const valgtRom = rom.find((romverdi) => romverdi.id === romId);
      onEndre({ ...hendelse, romId, omrade: valgtRom?.navn || "" });
    } else onEndre({ ...hendelse, romId: "", omrade: verdi });
  }
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      <Felt label="Hva ble gjort? *">
        <input
          autoFocus
          value={hendelse.tittel}
          onChange={(event) =>
            onEndre({ ...hendelse, tittel: event.target.value })
          }
          className="felt"
        />
      </Felt>
      <Felt label="Når?">
        <input
          ...
          value={hendelse.dato}
          onChange={(event) =>
            onEndre({ ...hendelse, dato: event.target.value })
          }
          className="felt"
        />
      </Felt>
      <Felt label="Hvor på boligen?">
        ...
        >
          ...
          {tidligereOmrade && (
            <option value={hendelse.omrade}>
              {hendelse.omrade} (tidligere valg)
            </option>
          )}
        </select>
      </Felt>
      <Felt label="Kostnad">
        <input
          inputMode="numeric"
          value={hendelse.kostnad || ""}
          onChange={(event) =>
            onEndre({ ...hendelse, kostnad: Number(event.target.value) || 0 })
          }
          className="felt"
        />
      </Felt>
      <Felt label="Utført av">
        <select
          value={hendelse.utfortAv || "tidligere-eier"}
          onChange={(event) =>
            onEndre({
              ...hendelse,
      ...
      {hendelse.utfortAv === "firma" && (
        <Felt label="Firma/håndverker">
          <input
            value={hendelse.firma}
            onChange={(event) =>
              onEndre({ ...hendelse, firma: event.target.value })
            }
            className="felt"
          />
        </Felt>
      ...
      <label className="text-sm font-semibold sm:col-span-2">
        ...
        <textarea
          rows={4}
          value={hendelse.beskrivelse}
          onChange={(event) =>
            onEndre({ ...hendelse, beskrivelse: event.target.value })
          }
          className="felt mt-2"
        />
      </label>
  ...
  return (
    <div className="mt-5 space-y-2">
      {krav.map((punkt) => (
        <label
          key={punkt.id}
          className="flex min-h-12 items-center gap-3 rounded-xl border border-stone-200 bg-white px-4 py-3 text-sm font-semibold"
        >
          <input
            type="checkbox"
  ...
  return (
    <label className="text-sm font-semibold">
      ...
      <input
        ...
        multiple
        accept={accept}
        onChange={(event) => onVelg(Array.from(event.target.files || []))}
        className="felt mt-2 text-sm"
      />
    </label>
  );
}
...
function Modal({
  ...
  return (
    <div
      className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 p-3"
      role="dialog"
      aria-modal="true"
    >
      <div className="mx-auto my-4 max-w-2xl rounded-3xl bg-white p-5 shadow-2xl sm:my-10 sm:p-7">
        <div className="mb-5 flex items-center justify-between gap-4">
          <h2 className="text-2xl font-bold">{tittel}</h2>
          <button
            type="button"
            onClick={onLukk}
            className="min-h-11 px-3 text-sm font-semibold text-slate-500"
          >
            Lukk
          </button>
        </div>
...
function typenavn(type: Arkivhendelse["arkivtype"]) {
  return type === "vedlikehold"
    ? "Utført vedlikehold"
    : type === "skader"
      ? "Løst skade/avvik"
      : "Oppussing og arbeid";
...
function maanedAar(dato: string) {
  if (!dato) return "Dato ikke registrert";
  try {
